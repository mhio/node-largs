...
const forEach = function(iterable, fn){
...
class Largs {
  ...
  arg( arg_name ){
    ...
    if ( arg_name === '_commands' ) throw new LargsException('Argument "_commands" is reserved')
  ...
  args( args ){
    forEach(args, (config, arg_name) => {
      // Trap commands
      if (arg_name === '_commands'){
        debug('found _commands')
        forEach(config, (command_arg_config, command_arg_name) => {
          debug('found command')
          this.command( command_arg_name, command_arg_config )
        })
        return 
      }
      // deprecate me for above `_commands` 202102
    ...
    function defaultHelpHandler( largs, help_arg ){
      ...
      if ( options.verbose ) {
        self.eachArgByGroup(arg => {
          let long  = (arg._long) ? `--${arg._long}`.padEnd(16) : ''.padEnd(16)
          let short = (arg._short) ? `-${arg._short}`.padEnd(3) : ''.padEnd(3)
          let type  = arg._type
