...
class Largs {
  ...
  args( args ){
    forEach(args, (config, argname) => {
      ...
      forEach(config, (value, argsetting) => {
        if (!o[argsetting]) throw new LargsException(`No method ${argsetting} to apply for ${argname}`, argname)
        o[argsetting](value)
      })
    })
    return this
  ...
  unknownError( str ){
    throw new ArgumentException(`The "${str}" argument is unknown`)
  }
  ...
  processArgv( argv ){
    ...
    while ( argv.length > 0 ) {
      ...
      if ( arg[0] !== '-' ) {
        ...
        // Unknown
        throw new LargsException('Unknown argument processing error', arg, argv)
      }
    }
