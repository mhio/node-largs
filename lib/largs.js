...
class Largs {
  ...
  // Setup an object of options/arguments
  //
  //     { what: { long: 'what', short: 'w', type: 'integer' }}
  ...
  /**
   * @summary Allow unknown arguments
   * @returns {undefined}
   */
  allowAnyArgs(){
    this.throw_on_unknown = false
    return
  }
  ...
  processShort( short, argv ){
    ...
    if ( !arg && this.throw_on_unknown ) this.unknownError(short)
  ...
  processLong( long, argv ){
    ...
    if ( !arg && this.throw_on_unknown ) this.unknownError(long)
  ...
  toJSON(){
    ...
    this.eachArg((arg, key) => {
      o.config[key] = this.config_options[key].toJSON()
      o.options[key] = this.config_options[key].getValue()
    })
